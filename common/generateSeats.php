...
if (!function_exists('generateSeatDiv')) {
...
{ 
    ...
    foreach ($obj as $key => $seat) {    
        if($isLastRow)
        {
            ...
            echo '<label class="" for="btn-' . $bus_id . $seat['name'] .'"  data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Tooltip on top"> <span class="'. genderStatus($seat['gender'],$seat['status']) .' active"> '. $seat['name'] .'</span></label>';  
            echo '</div>';
        }
        else
        {
